      ...
      <div className="scrollbar scrollbar-track-gray-700 scrollbar-thumb-gray-400 scrollbar-thumb-rounded-full scrollbar-track-rounded-full scrollbar-thin flex flex-row overflow-x-auto pb-4 sm:flex-col">
        <LayoutGroup id="a">
          {experienceData
            ...
            .map((job) => (
              <button
                className={`relative flex min-w-[110px] justify-center border-b-2 border-gray-300 px-6 py-2 text-sm outline-none duration-200 hover:bg-gray-300 dark:border-gray-600 dark:hover:bg-gray-700 sm:min-w-0 sm:justify-start sm:border-b-0 sm:border-l-2 lg:text-base ${
                  activeJob.id === job.id && "bg-gray-200 dark:bg-gray-800"
                }`}
                key={job.id}
              ...
              >
                ...
                {activeJob.id === job.id && (
                  <motion.div
                    className={
                      "absolute bottom-[-2px] left-0 z-10 h-[2px] w-full bg-blue-500 dark:bg-blue-500 sm:left-[-2px] sm:top-0 sm:h-full sm:w-[2px]"
                    }
                    layoutId="underline"
                  />
                )}
        ...
        <div>
          ...
          <ul className="ml-6 mt-4 space-y-2">
            {activeJob.description.map((desc, i) => (
              <li
                className="relative text-sm dark:text-gray-400 lg:text-base"
                key={i}
              >
                <p>{desc}</p>
                <span
